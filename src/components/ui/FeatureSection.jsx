...
export default function FeatureSection({ reverse, list, image }) {
  return (
    ...
    >
      <motion.div
        className='relative w-full lg:w-2/5 xl:w-1/2 lg:max-xl:aspect-4/3 aspect-7/4 rounded-xl overflow-clip shadow-md/40'
      ...
      <div className='w-full lg:w-3/5 xl:w-1/2 flex flex-col justify-center items-center'>
        ...
        >
          {list.map((item, index) => (
            <motion.li
              className='flex gap-2 items-center rounded-full bg-dark-600 w-fit py-1.5 pl-2 pr-4 shadow-md/40'
              key={index}
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
